"""
DataAgent — CSV/Excel Analysis UI with Claude
=============================================
"""
...
with st.sidebar:
    ...
    st.markdown("*CSV & Excel analysis powered by Claude*")
    st.divider()
    api_key = st.text_input("Anthropic API key", type="password", value=os.environ.get("ANTHROPIC_API_KEY", ""), placeholder="sk-ant-...")
    model = st.selectbox("Model", ["claude-sonnet-4-20250514", "claude-opus-4-20250514", "claude-haiku-4-5-20251001"])
    ...
    st.divider()
    chart_type = st.selectbox("Preferred chart", ["Auto (agent decides)", "Bar", "Line", "Scatter", "Histogram", "Box"])
...
def ask_claude(client, system, user):
    resp = client.messages.create(model=model, max_tokens=max_tokens, system=system, messages=[{"role":"user","content":user}])
    return resp.content[0].text

def build_chart(df, spec):
    kind, x_col, y_col, title, color = spec.get("type","bar").lower(), spec.get("x"), spec.get("y"), spec.get("title",""), spec.get("color")
    cols = df.columns.tolist()
    if x_col not in cols: x_col = cols[0] if cols else None
    if y_col and y_col not in cols: y_col = None
    try:
        if kind == "bar":
            if y_col: fig = px.bar(df, x=x_col, y=y_col, title=title, color=color)
            else:
                c = df[x_col].value_counts().reset_index(); c.columns = [x_col,"count"]
                fig = px.bar(c, x=x_col, y="count", title=title)
        elif kind == "line":   fig = px.line(df, x=x_col, y=y_col, title=title)
        elif kind == "scatter":fig = px.scatter(df, x=x_col, y=y_col, title=title, color=color)
        elif kind == "histogram": fig = px.histogram(df, x=x_col, title=title)
        elif kind == "box":    fig = px.box(df, x=x_col, y=y_col, title=title)
        else:
            c = df[x_col].value_counts().reset_index(); c.columns = [x_col,"count"]
            fig = px.bar(c, x=x_col, y="count", title=title)
        fig.update_layout(plot_bgcolor="white", paper_bgcolor="white", font_color="#1a1a2e", title_font_size=14, margin=dict(t=40,l=10,r=10,b=10))
        if kind in ("bar","histogram"): fig.update_traces(marker_color="#1D9E75")
        return fig
    except: return None
...
st.markdown("# 📊 DataAgent")
st.markdown("Upload a CSV or Excel file, then let Claude analyze it for you.")
...
if df is not None:
    ...
    with col_a: x_col = st.selectbox("X-axis / Category column", df.columns.tolist(), index=0)
    with col_b:
    ...
    focus = st.text_input("Focus question for Claude (optional)", placeholder="e.g. What drives revenue? Are there anomalies?")

    st.markdown("### Step 4 — Run the agent")
    if st.button("🚀  Analyze with Claude"):
        if not api_key: st.error("Please enter your Anthropic API key in the sidebar."); st.stop()
        ...
        with tabs[0]:
            if do_summary:
                ...
                st.markdown(text)
            else: st.info("Summary disabled.")

        with tabs[1]:
            if do_charts:
                with st.spinner("Planning charts…"):
                    raw = ask_claude(client,
                        "You are a data visualization expert. Return ONLY valid JSON, no explanation.",
                        f"Columns: {df.columns.tolist()}\nNumeric: {num_cols}\nCategorical: {cat_cols}\nUser x: {x_col}, y: {y_col}\nPreferred: {chart_type}\n\nReturn ONLY a JSON array of 1-3 chart specs. Each: {{\"type\":\"bar|line|scatter|histogram|box\",\"x\":\"col\",\"y\":\"col_or_null\",\"title\":\"...\",\"color\":\"col_or_null\"}}")
                try:
                    clean = raw.strip().replace("```json","").replace("```","").strip()
                    specs = json.loads(clean)
                    st.session_state.chart_specs = specs
                except: specs = [{"type":"bar","x":x_col,"y":y_col,"title":f"{x_col} distribution"}]
                for spec in specs:
                    fig = build_chart(df, spec)
                    if fig: st.plotly_chart(fig, use_container_width=True)
                    else: st.warning(f"Could not render: {spec.get('title','')}")
            else: st.info("Charts disabled.")

        with tabs[2]:
            if do_insights:
                with st.spinner("Generating insights…"):
                    raw_ins = ask_claude(client,
                        "You are a senior data analyst. Return ONLY a JSON array of 4-6 insight objects, each with: \"text\" (1-2 sentences), \"type\" (trend|warning|info), \"tag\" (short label).",
                ...
                except: insights = [{"text": raw_ins, "type":"info","tag":"General"}]
                for ins in insights:
                    kind = ins.get("type","info"); tag = ins.get("tag","insight")
                    badge_cls = {"trend":"badge-green","warning":"badge-amber"}.get(kind,"badge-blue")
                    st.markdown(f'<div class="insight-card {kind}"><span class="badge {badge_cls}">{tag}</span>{ins["text"]}</div>', unsafe_allow_html=True)
            else: st.info("Insights disabled.")

        with tabs[3]:
            if do_export:
                st.markdown("#### Export options")
                col1,col2,col3 = st.columns(3)
                col1.download_button("📥 Download data (CSV)", df.to_csv(index=False).encode(), "analyzed_data.csv", "text/csv", use_container_width=True)
                if st.session_state.summary_text:
                    col2.download_button("📄 Download summary (TXT)", st.session_state.summary_text.encode(), "summary.txt", "text/plain", use_container_width=True)
                if st.session_state.insights:
                    ins_df = pd.DataFrame(st.session_state.insights)
                    col3.download_button("💡 Download insights (CSV)", ins_df.to_csv(index=False).encode(), "insights.csv", "text/csv", use_container_width=True)
                if num_cols:
                    st.markdown("#### Numeric statistics")
                    stats_df = df[num_cols].describe().T.reset_index()
                    stats_df.columns = ["column"] + list(stats_df.columns[1:])
                    st.dataframe(stats_df, use_container_width=True)
                    st.download_button("📊 Download statistics (CSV)", stats_df.to_csv(index=False).encode(), "statistics.csv", "text/csv")
            else: st.info("Export disabled.")
else:
...
- **Surface insights** — trends, anomalies, correlations via Claude
